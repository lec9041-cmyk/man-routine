...
interface LinkedRoutine {
  ...
  title: string;
  icon: string;
  currentProgress: number; // 현재 달성률 (0-100)
  targetCount: number;
  currentCount: number;
}
...
interface Goal {
  ...
  endDate: string;
  color: string;
  linkedRoutines: LinkedRoutine[]; // 목표에 직접 연결된 루틴만 사용
  expanded?: boolean;
}
    ...
    setGoals((prev) =>
      prev.map((goal) =>
        goal.id === selectedGoalId
          ? { ...goal, linkedRoutines: [...goal.linkedRoutines, routine] }
          : goal
      )
    );
        ...
        <div className={`bg-gradient-to-br ${goal.color} p-2`}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 flex-shrink-0 ml-2">
              ...
              <button className="w-5 h-5 rounded-full bg-white/30 flex items-center justify-center hover:bg-white/40 transition-colors">
