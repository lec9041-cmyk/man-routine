...
interface Routine {
  ...
  targetCount: number; // 목표 횟수 (일일: 하루 N회, 주간: 주 N회, 월간: 월 N회)
  currentCount: number; // 현재 완료 횟수
  weeklyCount?: number; // 이번 주 누적 횟수 (주간용)
  monthlyCount?: number; // 이번 달 누적 횟수 (월간용)
  streak: number;
  bestStreak: number;
  color: string;
  timeOfDay: "morning" | "afternoon" | "evening" | "anytime";
  // 새로운 필드들
  repeatType: "forever" | "period"; // 계속 반복 or 기간 지정
  startDate?: string; // 시작일 (기간 지정 시)
  endDate?: string; // 종료일 (기간 지정 시)
  scheduleType: "count" | "specific"; // 횟수 지정 or 특정 날짜/요일 지정
  specificDays?: number[]; // 주간: 요일 (0=일요일, 1=월요일...) or 월간: 날짜 (1-31)
}
...
export function RoutineScreen({ onNavigate, shouldOpenAddModal, hideHeader }: RoutineScreenProps) {
  ...
  const groupedRoutines = {
    ...
    anytime: filteredRoutines.filter((r) => r.timeOfDay === "anytime"),
  };

  const TimeSection = ({ title, routines }: { title: string; routines: Routine[] }) => {
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 mb-1">
                        <span className="text-[9px] text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded-md font-medium">
                          {routine.category}
                        </span>
  ...
  return (
    <div className="min-h-screen bg-transparent pb-32">
      ...
      <div className="px-4 pt-3 pb-2 sm:px-5 sm:pt-4">
        ...
        {/* Routines by Time */}
        <TimeSection title="🌅 아침" routines={groupedRoutines.morning} />
        <TimeSection title="☀️ 오후" routines={groupedRoutines.afternoon} />
        <TimeSection title="🌙 저녁" routines={groupedRoutines.evening} />
        <TimeSection title="⏰ 언제든지" routines={groupedRoutines.anytime} />
      </div>
